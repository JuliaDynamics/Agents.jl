...
@testset "AStar" begin
    ...
    @testset "constructors" begin
        ...
        @test_throws AssertionError AStar(gspace;
        ...
        cost = AStar(gspace;
                     cost_metric = PenaltyMap(fill(1, 5, 5), MaxDistance{2}())).cost_metric
        @test typeof(cost) <: PenaltyMap{2}
        ...
        # ContinuousSpace
        @test_throws AssertionError AStar(cspace; walkmap = trues((10, 10)),
                                          cost_metric = PenaltyMap(fill(1, 5, 5)))
        @test_throws AssertionError AStar(cspace; walkmap = trues((10, 10)),
                                          cost_metric = PenaltyMap(fill(1, 5, 5)))
    ...
    @testset "API functions" begin
        ...
        @testset "ContinuousSpace" begin
            ...
            a = add_agent!((0.0, 0.0), model, (0.0, 0.0), 0.0)
            @test is_stationary(a, model.pf)
            ...
            @test all(isapprox.(a.pos, (0.75, 0.849999); atol)) ||
                  all(isapprox.(a.pos, (0.467156, 0.967156); atol))
            ...
            model.pf.walkmap[:, 3] .= 0
            @test all(get_spatial_property(random_walkable(model, model.pf),
                                           model.pf.walkmap, model) for _ in 1:10)
            ...
            a = add_agent!((0.0, 0.0), model, (0.0, 0.0), 0.0)
            @test all(plan_best_route!(a, [(2.5, 2.5), (4.99, 0.0), (0.0, 4.99)],
                                       model.pf) .≈ (2.5, 2.5))
            @test length(model.pf.agent_paths) == 1
            ...
            @test all(plan_best_route!(a, [(3.0, 0.3), (2.5, 2.5)], model.pf) .≈ (2.5, 2.5))
            @test isnothing(plan_best_route!(a, [(3.0, 0.3), (1.0, 0.1)], model.pf))
    ...
    @testset "metrics" begin
        pfinder_2d_np_m = AStar{2, false, true, Int64, DirectDistance{2}}(Dict(),
                                                                          (10, 10),
                                                                          copy(moore),
                                                                          0.0,
                                                                          trues(10, 10),
                                                                          DirectDistance{2}())
        pfinder_2d_np_nm = AStar{2, false, false, Int64, DirectDistance{2}}(Dict(),
                                                                            (10, 10),
                                                                            copy(vonneumann),
                                                                            0.0,
                                                                            trues(10, 10),
        ...
        pfinder_2d_p_m = AStar{2, true, true, Int64, DirectDistance{2}}(Dict(),
                                                                        (10, 10),
                                                                        copy(moore),
                                                                        0.0,
                                                                        trues(10, 10),
                                                                        DirectDistance{2}())
        pfinder_2d_p_nm = AStar{2, true, false, Int64, DirectDistance{2}}(Dict(),
                                                                          (10, 10),
                                                                          copy(vonneumann),
                                                                          0.0,
                                                                          trues(10, 10),
    ...
    @testset "pathing" begin
        ...
        pfinder_2d_np_m = AStar{2, false, true, Float64, DirectDistance{2}}(Dict(),
                                                                            (10.0, 10.0),
                                                                            copy(moore),
                                                                            0.0,
                                                                            wlk,
        ...
        pfinder_2d_np_nm = AStar{2, false, false, Float64, DirectDistance{2}}(Dict(),
                                                                              (10.0, 10.0),
                                                                              copy(vonneumann),
                                                                              0.0,
                                                                              wlk,
        ...
        pfinder_2d_p_m = AStar{2, true, true, Float64, DirectDistance{2}}(Dict(),
                                                                          (10.0, 10.0),
                                                                          copy(moore),
                                                                          0.0,
                                                                          wlk,
                                                                          DirectDistance{2}())
        pfinder_2d_p_nm = AStar{2, true, false, Float64, DirectDistance{2}}(Dict(),
                                                                            (10.0, 10.0),
                                                                            copy(vonneumann),
                                                                            0.0,
                                                                            wlk,
        ...
        # Continuous
        model = ABM(Agent6, ContinuousSpace((10.0, 10.0)))
        p = collect(Pathfinding.find_continuous_path(pfinder_2d_np_m, (0.25, 0.25),
                                                     (7.8, 9.5)))
        testp = [
            (0.71429, 2.5),
            (0.71429, 4.16667),
        ...
        p = collect(Pathfinding.find_continuous_path(pfinder_2d_p_m, (0.25, 0.25),
                                                     (7.8, 9.5)))
        testp = [
            (2.14286, 9.16667),
            (3.57143, 9.16667),
