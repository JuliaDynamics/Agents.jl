@testset "CSV" begin
    ...
    function Models.SchellingAgent(id, p1, p2, mood, group)
        Models.SchellingAgent(id, (p1, p2), mood, group)
    end
    ...
    @test all(model[i].previous_opinion == empty_model[i].previous_opinion
              for i in allids(model))

    genocide!(empty_model)
    AgentsIO.populate_from_csv!(empty_model, "test.csv", HKAgent,
                                Dict(:id => 1, :op1 => 3, :op2 => 2))
    ...
    AgentsIO.dump_to_csv("test.csv", HKAgent[model[i] for i in 1:nagents(model)],
                         [:old_opinion])
    ...
    genocide!(empty_model)
    AgentsIO.populate_from_csv!(empty_model, "test.csv", HKAgent,
                                Dict(:op1 => 1, :op2 => 1); row_number_is_id = true)
