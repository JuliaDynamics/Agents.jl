...
@testset "OpenStreetMap space" begin
    ...
    @testset "Obtaining positions" begin
        ...
        intersection_diff = maximum(abs.(OSM.lonlat(start_i, model) .- start_lonlat))
        start_road = OSM.nearest_road(start_lonlat, model)
        road_diff = maximum(abs.(OSM.lonlat(start_r, model) .- start_lonlat))
