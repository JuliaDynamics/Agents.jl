...
@testset "JLD2" begin
    function test_model_data(model, other)
        @test (model.scheduler isa Function && model.scheduler == other.scheduler) ||
              (!isa(model.scheduler, Function) &&
               typeof(model.scheduler) == typeof(other.scheduler))
    ...
    function test_space(space::GridSpace, other)
        ...
        @test all(length(other.stored_ids[pos]) == length(space.stored_ids[pos])
                  for pos in eachindex(space.stored_ids))
        @test all(all(x in other.stored_ids[pos] for x in space.stored_ids[pos])
                  for pos in eachindex(space.stored_ids))
    ...
    function test_space(space::GridSpaceSingle, other)
        @test size(space) == size(other)
        @test all(other.stored_ids[pos] == space.stored_ids[pos]
                  for pos in eachindex(space.stored_ids))
        @test space.metric == other.metric
    end
    ...
    function test_costmetric(metric::Pathfinding.DirectDistance{D},
                             other::Pathfinding.DirectDistance{D}) where {D}
        @test metric.direction_costs == other.direction_costs
    end

    function test_costmetric(metric::Pathfinding.MaxDistance{D},
                             other::Pathfinding.MaxDistance{D}) where {D}
        @test true
    end

    function test_costmetric(metric::Pathfinding.PenaltyMap{D},
                             other::Pathfinding.PenaltyMap{D}) where {D}
    ...
    @testset "No space" begin
        model = ABM(Agent2, nothing; properties = Dict(:abc => 123),
    ...
    @testset "GridSpaceSingle" begin
        function schelling_single(; numagents = 320, griddims = (20, 20),
                                  min_to_be_happy = 3)
            @assert numagents < prod(griddims)
            space = GridSpaceSingle(griddims, periodic = false)
            properties = Dict(:min_to_be_happy => min_to_be_happy)
            model = ABM(Models.SchellingAgent, space; properties,
    ...
    @testset "ContinuousSpace" begin
        ...
        @test all(model[i].separate_factor == other[i].separate_factor
                  for i in allids(model))
        @test all(model[i].match_factor == other[i].match_factor for i in allids(model))
        @test all(model[i].visual_distance == other[i].visual_distance
                  for i in allids(model))
        # model data
        test_model_data(model, other)
        # space data
    ...
    @testset "GraphSpace" begin
        ...
        for i in 1:30
            add_agent_pos!(Agent7(i, i % 10 + 1, rand(model.rng) < 0.5,
                                  rand(model.rng, Int)), model)
        end
        ...
        @test all(length(model.space.stored_ids[pos]) ==
                  length(other.space.stored_ids[pos])
                  for pos in eachindex(model.space.stored_ids))
        @test all(all(x in other.space.stored_ids[pos] for x in model.space.stored_ids[pos])
                  for pos in eachindex(model.space.stored_ids))
    ...
    @testset "Grid Pathfinder" begin
        ...
        test_pathfinding_model(setup_model(;
                                           cost_metric = Pathfinding.PenaltyMap(pmap,
                                                                                direct))...)
        test_pathfinding_model(setup_model(;
                                           cost_metric = Pathfinding.PenaltyMap(pmap, maxd))...)
        test_pathfinding_model(setup_model(;
                                           cost_metric = Pathfinding.PenaltyMap(pmap, hmm))...)
    ...
    @testset "Continuous Pathfinder" begin
        ...
        test_pathfinding_model(setup_model(cost_metric = Pathfinding.PenaltyMap(pmap,
                                                                                direct))...)
    ...
    @testset "Multi-agent" begin
        ...
        @test all(haskey(other.agents, i) for i in allids(model))
        @test all(model[i].pos == other[i].pos for i in allids(model))
        @test all(model[i].weight == other[i].weight
                  for i in allids(model) if model[i] isa Agent3)
        # model data
        test_model_data(model, other)
        # space data
        test_space(model.space, other.space)
    ...
    @testset "OSMSpace" begin
        @agent Zombie OSMAgent begin infected::Bool end
        ...
        @test nagents(other) == nagents(model)
        @test all(haskey(other.agents, i) for i in allids(model))
        @test all(OSM.latlon(model[i].pos, model) == OSM.latlon(other[i].pos, other)
                  for i in allids(model))
        @test all(model[i].infected == other[i].infected for i in allids(model))
        # model data
        test_model_data(model, other)
        @test sort(collect(keys(model.space.routes))) ==
              sort(collect(keys(other.space.routes)))
        @test all(model.space.routes[i].route == other.space.routes[i].route
                  for i in keys(model.space.routes))
        @test all(model.space.routes[i].start == other.space.routes[i].start
                  for i in keys(model.space.routes))
        @test all(model.space.routes[i].dest == other.space.routes[i].dest
                  for i in keys(model.space.routes))
        @test all(model.space.routes[i].return_route == other.space.routes[i].return_route
                  for i in keys(model.space.routes))
        @test all(model.space.routes[i].has_to_return ==
                  other.space.routes[i].has_to_return for i in keys(model.space.routes))
