...
@testset "Simple Schedulers" begin
    ...
    # Schedulers.by_id
    model = StandardABM(Agent0; scheduler = Schedulers.by_id, warn_deprecation = false)
    for i in 1:N
        add_agent!(model)
    end
    ...
    # by property
    model = StandardABM(Agent2; scheduler = Schedulers.by_property(:weight), warn_deprecation = false)
    for i in 1:N
...
@testset "Mixed Scheduler" begin
    ...
    Random.seed!(12)
    model =
        StandardABM(Union{Agent1,Agent0}, scheduler = Schedulers.by_type((Agent1, Agent0), true), warn = false, warn_deprecation = false)
    for id in 1:3
        add_agent!(Agent1, model, (0, 0))
    end
    for id in 4:6
