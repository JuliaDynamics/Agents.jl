...
@testset "Scheduler tests" begin
    ...
    function init_mixed_model(choices = [3, 3, 3, 3]; scheduler = Schedulers.fastest)
        model = ABM(Union{Agent0, Agent1, Agent2, Agent3}, scheduler = scheduler,
                    warn = false)
    ...
    @testset "Mixed Scheduler" begin
        ...
        @test unique([typeof(model[id]) for id in s1]) !=
              unique([typeof(model[id]) for id in s2])
        ...
        Random.seed!(12)
        model = ABM(Union{Agent1, Agent0},
                    scheduler = Schedulers.by_type((Agent1, Agent0), true), warn = false)
        for id in 1:3
            a1 = Agent1(id, (0, 0))
            add_agent!(a1, model)
        end
    ...
    @testset "Non-allocating schedulers" begin
        ...
        # shuffling types scheduler
        Random.seed!(12)
        model = init_mixed_model2(scheduler = Schedulers.ByType(true, false,
                                                                Union{Agent0, Agent1,
                                                                      Agent2, Agent3}))
        s1 = model.scheduler(model)
        ...
        # NOT shuffling types scheduler
        Random.seed!(12)
        model = init_mixed_model2(scheduler = Schedulers.ByType(false, false,
                                                                Union{Agent0, Agent1,
                                                                      Agent2, Agent3}))
        s1 = model.scheduler(model)
        s2 = model.scheduler(model)
        ...
        # Not shuffling types, but shuffling agents
        Random.seed!(12)
        model = init_mixed_model2(scheduler = Schedulers.ByType(false, true,
                                                                Union{Agent0, Agent1,
                                                                      Agent2, Agent3}))
        ...
        Random.seed!(12)
        model = ABM(Union{Agent1, Agent0},
                    scheduler = Schedulers.ByType((Agent1, Agent0), true), warn = false)
        for id in 1:3
            a1 = Agent1(id, (0, 0))
            add_agent!(a1, model)
        end
