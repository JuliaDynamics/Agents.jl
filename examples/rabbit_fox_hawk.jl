...
function initialize_model(heightmap_url =
    ...
    grass = BitArray(rand(rng, dims[1:2]...) .<
                     ((grass_level .- heightmap) ./ (grass_level - water_level)))
    properties = (
    ...
    for _ in 1:n_rabbits
        add_agent_pos!(Rabbit(nextid(model), ## Using `nextid` prevents us from having to manually keep track
                              ## of animal IDs
    ...
    for _ in 1:n_foxes
        add_agent_pos!(Fox(nextid(model),
                           random_walkable(model, model.landfinder),
                           rand(model.rng, Δe_rabbit:(2Δe_rabbit))),
                       model)
    end
    for _ in 1:n_hawks
        add_agent_pos!(Hawk(nextid(model),
...
function rabbit_step!(rabbit, model)
    ...
    predators = [x.pos
                 for x in nearby_agents(rabbit, model, model.rabbit_vision)
                 if
    ...
    if !isempty(predators) && is_stationary(rabbit, model.landfinder)
        ...
        if all(direction .≈ 0.0)
            ## Move anywhere
            chosen_position = random_walkable(rabbit.pos, model, model.landfinder,
                                              model.rabbit_vision)
        else
            ...
            chosen_position = random_walkable(position, model, model.landfinder,
                                              model.rabbit_vision / 2.0)
        end
...
function hawk_step!(hawk, model)
    ...
    if is_stationary(hawk, model.airfinder)
        prey = [x
                for x in nearby_agents(hawk, model, model.hawk_vision) if x.type == :rabbit]
        if isempty(prey)
            plan_route!(hawk,
...
function model_step!(model)
    ...
    growable = view(model.grass,
                    model.grass .== 0 .& model.water_level .< model.heightmap .<=
                    model.grass_level)
