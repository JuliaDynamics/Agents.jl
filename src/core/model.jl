    ...
    if space !== nothing
        ...
        elseif space_type <: ContinuousSpace || space_type <: ContinuousSpace
            if !(pos_type <: SVector{D,<:AbstractFloat} where {D})
                throw(ArgumentError("`pos` field in Agent struct must be of type `SVector{<:AbstractFloat}` when using ContinuousSpace."))
            end
            if warn &&
               any(isequal(:vel), fieldnames(A)) &&
               !(fieldtype(A, :vel) <: SVector{D,<:AbstractFloat} where {D})
                @warn "`vel` field in Agent struct should be of type `SVector{<:AbstractFloat}` when using ContinuousSpace."
            end
        end
    end
end
