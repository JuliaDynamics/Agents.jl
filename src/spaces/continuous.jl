...
- `rho_obstacle`: Positive repulsive gradient for obstacle; similar to `rho` but used for agent-obstacle repulsive forces.
...
once per iteration rather than every time `fmp_update_vel!` is called.
...
`interacting_pairs` each time `fmp_update_vel!` is called. 
"""
...
- Obstacle force: similar to repulsive force but generated by proximity to obstacle in the state space
...
function repulsive_force(model::AgentBasedModel, agent)
    ...
    # targets/obstacles do not experience repulsive feedback
