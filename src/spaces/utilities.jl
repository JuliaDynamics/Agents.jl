export euclidean_distance, manhattan_distance, get_direction, normalize_position, walk!,
       spacesize
...
"""
function euclidean_distance(a::A,
                            b::B,
                            model::ABM{<:Union{ContinuousSpace, AbstractGridSpace}}) where {
                                                                                            A <:
                                                                                            AbstractAgent,
                                                                                            B <:
                                                                                            AbstractAgent
                                                                                            }
    euclidean_distance(a.pos, b.pos, model)
end

function euclidean_distance(p1::ValidPos,
                            p2::ValidPos,
                            model::ABM{
                                       <:Union{ContinuousSpace{D, false},
                                               AbstractGridSpace{D, false}}}) where {D}
    sqrt(sum(abs2.(p1 .- p2)))
end

function euclidean_distance(p1::ValidPos,
                            p2::ValidPos,
                            model::ABM{<:ContinuousSpace{D, true}}) where {D}
    total = 0.0
...
function euclidean_distance(p1::ValidPos, p2::ValidPos,
                            model::ABM{<:AbstractGridSpace{D, true}}) where {D}
    total = 0.0
...
sense: currently `AbstractGridSpace` and `ContinuousSpace`.
"""
function manhattan_distance(a::A,
                            b::B,
                            model::ABM{<:Union{ContinuousSpace, AbstractGridSpace}}) where {
                                                                                            A <:
                                                                                            AbstractAgent,
    ...
    manhattan_distance(a.pos, b.pos, model)
end

function manhattan_distance(p1::ValidPos,
                            p2::ValidPos,
                            model::ABM{
                                       <:Union{ContinuousSpace{D, false},
                                               AbstractGridSpace{D, false}}}) where {D}
...
function manhattan_distance(p1::ValidPos,
                            p2::ValidPos,
                            model::ABM{
                                       <:Union{ContinuousSpace{D, true},
                                               AbstractGridSpace{D, true}}}) where {D}
    total = 0.0
...
function get_direction(from::NTuple{D, Float64},
                       to::NTuple{D, Float64},
                       space::Union{ContinuousSpace{D, true}, AbstractGridSpace{D, true}}) where {
                                                                                                  D
                                                                                                  }
    best = to .- from
...
function nearby_positions(position::Integer,
...
"""
function walk!(agent::AbstractAgent,
               direction::NTuple{D, Int},
               model::ABM{<:AbstractGridSpace};
               ifempty::Bool = true) where {D}
...
function walk!(agent::AbstractAgent,
               ...
               model::ABM{<:ContinuousSpace}) where {D}
    target = normalize_position(agent.pos .+ direction, model)
    move_agent!(agent, target, model)
end
...
"""
function walk!(agent, ::typeof(rand), model::ABM{<:AbstractGridSpace{D}};
               kwargs...) where {D}
