...
function Agents.plan_route!(agent::A,
                            dest::Dims{D},
                            pathfinder::AStar{D}) where {D, A <: AbstractAgent}
...
"""
function nearby_walkable(position, model::ABM{<:GridSpace{D}}, pathfinder::AStar{D},
                         r = 1) where {D}
    Iterators.filter(x -> pathfinder.walkmap[x...] == 1,
...
function random_walkable(model::ABM{<:GridSpace{D}}, pathfinder::AStar{D}) where {D}
    return Tuple(rand(model.rng,
                      filter(x -> pathfinder.walkmap[x],
                             CartesianIndices(model.space.stored_ids))))
end
